...
We implement PIPE-Cypher as a Python package around the boundary an enterprise deployment would actually use: a read-only graph client, a schema and value profiler, a local model endpoint, deterministic Cypher governance, and export and audit tools. Neo4j is the execution backend in our experiments, but the paper's unit of study is Cypher over property graphs, not a single vendor product. In the code, the graph client is the only backend-specific layer; the generator, validators, judge interface, diversity metrics, and evidence ledger operate on a normalized property-graph profile, generated Cypher, execution traces, and validation records. Reported benchmark generation and judge review use a local Qwen3.5-9B endpoint~\citep{qwen2026qwen35hf} behind a vLLM/OpenAI-compatible interface. Downstream evaluation separately tests 11 completed locally served checkpoints spanning general instruction, code-tuned, Cypher-tuned, and Text2Cypher-tuned families. This separation is intentional: generation and judging measure benchmark construction with one deployed local endpoint, while downstream transfer asks how other local models behave when they face the exported benchmark. All generation and evaluation run inside the organization's compute boundary without paid generation APIs.

A graph profile is more than a schema dump. It is the contract that tells the rest of the pipeline which labels, relationship directions, properties, categorical literals, and value samples can be used safely. The built-in FinBench profile is grounded in the public datagen snapshot export and includes typed node and relationship properties. Live schema inspection also records bounded low-cardinality string values as categorical constraints for prompting and validation. The generated Neo4j import script uses uniqueness constraints for nodes and creates, rather than merges, transaction relationships so repeated account-to-account events remain visible to path and aggregation queries. The SNB reference profile is grounded in the official Neo4j/Cypher headers and read-query files. These details matter because benchmark generation depends on what the execution engine will actually see: whether a property belongs to a node or edge, whether an account pair has multiple transaction events, whether a string value is safe enough to use as a literal, and which relationship directions are legal. The same profiling contract is what a company-owned onboarding run produces before any LLM call.
...
The public code repository is available at \url{https://github.com/suraj-ranganath/PIPE-Cypher}. The public-proxy benchmark exports for FinBench/SNB and ICIJ Offshore Leaks are available at \url{https://huggingface.co/datasets/suraj-ranganath/PIPE-Cypher-benchmarks}. These artifacts are linked only in the non-anonymous arXiv version; the EMNLP review submission uses anonymous supplementary material.
