...
One might ask why a general AI agent cannot build this benchmark directly. For a small one-off study, a strong model with tools can inspect a schema, draft Cypher, execute it, and revise. Enterprise refreshes recur as schemas and workflows change, often with smaller local models chosen for cost, latency, and privacy. PIPE-Cypher constrains the loop: the model writes questions and Cypher, while graph checks ground values, preserve directions, reject unsafe or ambiguous examples, balance coverage, and leave an audit trail.
