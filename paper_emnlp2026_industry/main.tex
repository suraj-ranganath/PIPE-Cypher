...
\section{Appendix Roadmap and Artifact Summary}

The appendix is organized as a claim-evidence trace. Each table or figure appears next to the paragraph that interprets it so the reader does not need to reconstruct the argument from a block of floats. The opening tables summarize the paper-facing run setup and exported artifact before the appendix turns to reliability, onboarding, downstream transfer, diversity, governance, prompt contracts, and representative examples.

\subsection{Benchmark Artifact Summary}

Tables~\ref{tab:experiment_plan}, \ref{tab:benchmark_export}, and \ref{tab:full_artifact_distribution} give the setup, export manifest summary, and gate/distribution details that are compressed in the six-page main body.
...
The first empirical claim is reliability: once reverse grounding and deterministic Cypher governance are in place, PIPE-Cypher fills every planned graph/category cell at target scale.
...
Table~\ref{tab:supported_text_metrics} lists optional text-overlap metrics for debugging answer rendering and near-match behavior. These are not correctness metrics; the paper's claims use execution accuracy and answer-set F1.
...
\FloatBarrier
\clearpage
\input{appendix_review_response_matrix}
\FloatBarrier
\clearpage
