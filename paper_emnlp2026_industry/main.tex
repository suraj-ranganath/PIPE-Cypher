...
The strongest ablation result is not that every optional component independently increases yield; it is that once reverse grounding and deterministic Cypher governance are in place, the benchmark factory reliably fills every planned graph/category cell at target scale. Figure~\ref{fig:ablation_suite_target100_main} gives the full stress-baseline evidence behind the compact RQ2 discussion in the main paper. The unconstrained local-generation rows are intentionally shown as a stress baseline: they demonstrate why executable gates and category control are necessary for enterprise benchmark generation.
...
The exported artifact is designed to make scale and balance visible. Figure~\ref{fig:full_export_distribution} shows the planned 2:1 FinBench/SNB mix, exact category balance, and near-even difficulty split; Tables~\ref{tab:graph_statistics}--\ref{tab:icij_onboarding} then show that the same machinery also onboards ICIJ Offshore Leaks, a larger public finance/compliance graph outside the two LDBC study workloads.
