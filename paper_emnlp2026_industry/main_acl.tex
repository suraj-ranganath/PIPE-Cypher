...
\noindent\textbf{RQ3: downstream utility.} We ran a downstream Text2Cypher evaluation using local Qwen3.5-9B on the exported full test split. The model saw schema text and the natural-language question, generated Cypher, and was evaluated by live execution against the corresponding FinBench or SNB database. Figure~\ref{fig:downstream_breakdown_main} shows why execution-grounded benchmarks are useful: parse validity, schema validity, and execution success can be high while exact execution accuracy remains low for operational categories. The zero-shot 9B baseline solves simple aggregation examples but struggles with retrieval, boolean, negation, and path-shaped questions.

\input{tables_downstream_evaluation}

\begin{figure*}[t]
\centering
\includegraphics[width=0.78\textwidth]{figures/downstream_breakdown.pdf}
\caption{Downstream zero-shot Qwen3.5-9B Text2Cypher evaluation by workload category on the full test split. The benchmark separates parse/schema/execution success from exact execution accuracy, exposing operational categories where executable Cypher is still semantically wrong.}
\label{fig:downstream_breakdown_main}
\end{figure*}
...
\caption{Audited target-100 ablation suite over FinBench and SNB. Every non-unconstrained graph/setting cell reaches all eight category targets with 800 accepted examples per cell.}
\label{fig:ablation_suite_target100}
\end{figure}
