...
Enterprise Text2Cypher evaluation is rarely a one-time leaderboard exercise. A team needs to know whether a model is safe on its own graph: whether it follows local relationship directions, uses approved values, avoids writes, returns the context analysts expect, and fails in a way that can be debugged. Public benchmarks are useful for measuring general ability, but they cannot answer that deployment question because the schema, value policy, business vocabulary, and common questions are private and keep changing. We therefore view benchmark generation as part of the deployment stack: a repeatable way to turn a live graph into a controlled evaluation set before users rely on generated Cypher.

In that setting, PIPE-Cypher is meant to be rerun when an enterprise graph changes. Each example records the schema snapshot, graph profile, model identifier, validation gates, execution sample, judge scores, difficulty features, and source run. The audit trail lets a team ask concrete operational questions: whether a failure came from schema retrieval, value grounding, prompt constraints, graph execution, or the model itself. Teams can refresh after schema changes, compare models on the same held-out split, inspect failures by category, build a schema-specific question-answer bank for retrieval or adaptation, and export redacted review packets. A deployment needs read-only graph credentials, schema introspection, a bounded value policy, a local endpoint, a dry run, scaled generation, judge calibration, and redacted export. We validate this pattern on FinBench, SNB, and ICIJ while keeping the private-tenant gap explicit.
