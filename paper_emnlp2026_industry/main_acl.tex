...
\noindent\textbf{Diversity and residual concentration.} We report diversity as an audit signal, not as a slogan. The full export has perfect category balance and near-perfect difficulty balance, uses 1,115 unique grounded entity values, and exactly quotes grounded values in 82.6\% of examples with entity bindings. The reported local-model run still has low query-signature diversity because seeded graph-grounded templates are doing substantial work. A post-hoc diversity-governed selector improves structural substructure coverage, adjusted Distinct-2, query-signature ratio, and property coverage at the same graph/category target (Appendix Table~\ref{tab:diversity_improvement}), while also showing that source-pool signature scarcity remains the next generation target. This supports the intended industry workflow: PIPE-Cypher can produce a balanced executable benchmark, and it also exposes the residual template and value concentration that a benchmark owner should monitor during refresh or fine-tuning data generation.
