...
- When benchmarking model families inspired by older public Text2Cypher charts, use the latest locally runnable model in that family for new runs. Older chart entries may be kept as historical baselines if already launched, but new reviewer-facing comparisons should prefer current local-weight successors such as Gemma 3-family models over Gemma 2, Qwen3-Coder-family models over older code baselines, and current Llama instruct checkpoints when license access and GPU memory permit.
