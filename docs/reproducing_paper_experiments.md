...
The public benchmark exports used in the paper are available at
[suraj-ranganath/PIPE-Cypher-benchmarks](https://huggingface.co/datasets/suraj-ranganath/PIPE-Cypher-benchmarks).
To reproduce results from scratch, load the graphs below, serve the listed
local models, run the configs at the stated targets, and regenerate the
summaries with the scripts in this repository.
