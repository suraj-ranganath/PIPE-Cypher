...
- Deterministic tests pass: `146 passed`.
...
- `scripts/collect_remote_ablation_suite.py` can wait for a remote tmux session to finish, fetch completed `ds-serv6` ablation run directories by run prefix, copy the remote log into the local snapshot directory, and invoke the local summary/audit renderer. This is needed for the active target-50 run because it was launched from an older remote checkout before audit output was added to the live runner.
