...
- Scaled ablation summarization now exists: `scripts/summarize_live_ablation_suite.py` creates JSON/Markdown audit packets from run directories, detects missing or incomplete graph/variant cells, computes category-target coverage, and refuses LaTeX paper-table rendering from incomplete suites by default.
